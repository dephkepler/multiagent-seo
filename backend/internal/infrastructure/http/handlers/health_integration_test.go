...
func TestHealthz_Integration(t *testing.T) {
	...
	router := apihttp.NewRouter(
		...
		handlers.NewServer(handlers.NewHealthHandler(svc), handlers.NewWordpressSitesHandler(nil), handlers.NewLoginHandler(nil), handlers.NewArticlesHandler(nil), handlers.NewLinkbuildingHandler(nil)),
		nil,
	)
